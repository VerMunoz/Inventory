...
function GatherInformation {
	...
	# GENERAL SERVER INFORMATION
		...
		MEMORY=$(svmon -G | awk ' /memory/ {printf $2}')
		MEMORY_USED=$(svmon -G | awk ' /memory/ {printf $3}')
		MEMORY_AVAILABLE=MEMORY-MEMORY_USED
	    SWAP=$(svmon -G | awk ' /pg space/ {printf $3}')
		SWAP_USED=$(svmon -G | awk ' /pg space/ {printf $4}')
	...
	## FALTA System model= virtual o físico Proccesor Type, Memory Used, Memory Available, SWAP FREE, STORAGE 
    ...
    # STORAGE 
	    DEVICES=$(df -h -x tmpfs | grep /dev/ | awk '{print $1}')
     	for i in $DEVICES; do
		 	printf "\t$i\n"
			SIZE=$(df -h  | grep $i | awk '{print "Size:" $2 "GB"}')
			USED=$(df -h  | grep $i | awk '{print "Used:" $3 "GB"}')
			USE=$(df -h   | grep $i | awk '{print "Porcentaje de uso:" $5}' | sed s/\%//g )
			MOUNT=$(df -h | grep $i | awk '{print "Mount:" $6}')
			printf "\t\t$SIZE\n\t\t$USED\n\t\t$USE\n\t\t$MOUNT\n\n"
	    done
        
         	printf "NETWORK\n  Interfaces:\n"
	# NETWORK SETTINGS
			INTERFACES=$(ifconfig -a | awk '{print $1}'| sed '1d')
			for i in $INTERFACES; do
				printf "\t$i\n"
	...
	## FALTA Memory Used, Memory Available, STORAGE 
    ;;
	...
	# GENERAL SERVER INFORMATION
		...
		OS_LEVEL=$(uname -r)
		SYSTEM_MODEL=$(swlist |grep HPUX11i)
		OS_MODEL=$(osinfo)
		PROCESSOR_TYPE=$(kctune)
		PROCESSORS=$(machinfo)
		CLOCK=
		MEMORY=$(machinfo |grep -i memory)
		MEMORY_USED=
		MEMORY_AVAILABLE=
		SWAP=$(swapinfo)
		SWAP_FREE=$(swapinfo)
	...
	# STORAGE 
	   LUN_DISK=$(ioscan -N -fnu -C disk)
	   HP_XP_STORAGE=$(xpinfo -i)
	   printf "LUN DISK: $LUN_DISK\n HP XP STORAGE $HP_XP_STORAGE\n\n"
